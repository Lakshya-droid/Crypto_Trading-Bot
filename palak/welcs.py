...
class Login():
    def __init__(self,root1):
        ...
        Frame_login.place(x=150,y=150,height=380,width=500)
        ...
        lb1_bitmex_sec_key = Label(Frame_login, text="Dark Mode", font=("Goudy old style", 14, "bold"), fg="gray",
        ...
        off_button2.place(x=280,y=280)
        on_button2 = Radiobutton(Frame_login, text="ON", variable=self.dmv,indicatoron=False, value="on", width=8)
        on_button2.place(x=220, y=280)
        nuser=Button(Frame_login,text="New User -> Log-In",bg="white",fg="#d77337",bd=0,font=("times new roman" ,12),command=lambda: self.openNewWindow()).place(x=90,y=320)
        Login_btm = Button(self.root1 ,text="Login", bg="#d77337", fg="white",font=("times new roman", 12),command=lambda: self.signinbut()).place(x=300, y=510,width=180,height=40)
    def signinbut(self):
        global h
        ...
        if self.txt_ps.get()=="" or self.txt_user.get()=="":
            ...
            if decr(pas[idd.index(a)])==str(b):
                ...
                if self.df.test[c]=="test":
                    val=True
                else:
                    val=False
                df1=pd.DataFrame([[self.bpk,self.bsk,self.btpk,self.btsk]],columns=["bpk","bsk","btpk","btsk"])
    ...
    def mainWindow(self):
        ...
        self.binance = BinanceClient(self.bpk,self.bsk,testnet=True, futures=True)
        self.bitmex = BitmexClient(self.btpk,self.btsk,testnet=True)
        self.root1.destroy()

        self.root = Root(self.binance, self.bitmex,self.dmv.get())
