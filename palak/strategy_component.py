...
class StrategyEditor(tk.Frame):
    def __init__(self, root: "Root", binance: BinanceClient, bitmex: BitmexClient,mod ,*args, **kwargs):
        ...
        self._commands_frame = tk.Frame(self, bg=self.BG_COLOR)
        self._commands_frame.pack(side=tk.TOP)

        self._table_frame = tk.Frame(self, bg=self.BG_COLOR)
        self._table_frame.pack(side=tk.TOP)

        self._add_button = tk.Button(self._commands_frame, text="Add strategy", font=self.GLOBAL_FONT,
                                     command=self._add_strategy_row, bg=self.BG_COLOR_2, fg=self.FG_COLOR)
        self._add_button.pack(side=tk.TOP)

        self.body_widgets = dict()

        self._headers_frame = tk.Frame(self._table_frame, bg=self.BG_COLOR)
        ...
        self._base_params = [
            {"code_name": "strategy_type", "widget": tk.OptionMenu, "data_type": str,
             "values": ["Technical", "Breakout"], "width": 10, "header": "Strategy","hw" : 10},
            {"code_name": "contract", "widget": tk.OptionMenu, "data_type": str, "values": self._all_contracts,
             "width": 15, "header": "Contract","hw" : 10},
            {"code_name": "timeframe", "widget": tk.OptionMenu, "data_type": str, "values": self._all_timeframes,
             "width": 10, "header": "Timeframe","hw" :10 },
            {"code_name": "balance_pct", "widget": tk.Entry, "data_type": float, "width": 10, "header": "Balance %","hw" : 8},
            {"code_name": "take_profit", "widget": tk.Entry, "data_type": float, "width": 7, "header": " TP %","hw" : 7},
            {"code_name": "stop_loss", "widget": tk.Entry, "data_type": float, "width": 7, "header": "SL %","hw" : 8},
            {"code_name": "parameters", "widget": tk.Button, "data_type": float, "text": "Parameters",
             "bg": self.BG_COLOR_2, "command": self._show_popup, "header": "", "width": 10 , "hw" : 10},
            {"code_name": "activation", "widget": tk.Button, "data_type": float, "text": "OFF",
             "bg": "darkred", "command": self._switch_strategy, "header": "", "width" : 8, "hw" :10 },
            {"code_name": "delete", "widget": tk.Button, "data_type": float, "text": "X",
             "bg": "darkred", "command": self._delete_row, "header": "", "width": 6,"hw" : 10},
        ...
        for idx, h in enumerate(self._base_params):
            header = tk.Label(self._headers_frame, text=h['header'], bg=self.BG_COLOR, fg=self.FG_COLOR, font=self.GLOBAL_FONT,
                              width=h['hw'], bd=1, relief=tk.FLAT)
            header.grid(row=0, column=idx, padx=2)

        header = tk.Label(self._headers_frame, text="", bg=self.BG_COLOR, fg=self.FG_COLOR, font=self.GLOBAL_FONT,
        ...
        self._body_frame = ScrollableFrame(self._table_frame, bg=self.BG_COLOR, height=250)
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.OptionMenu:
                ...
                self.body_widgets[code_name][b_index] = tk.Entry(self._body_frame.sub_frame, justify=tk.CENTER,
                                                                 bg=self.BG_COLOR_2, fg=self.FG_COLOR,
                                                                 font=self.GLOBAL_FONT, bd=1, width=base_param['width'])
                ...
                self.body_widgets[code_name][b_index] = tk.Button(self._body_frame.sub_frame, text=base_param['text'],
                                        bg=base_param['bg'], fg=self.FG_COLOR, font=self.GLOBAL_FONT, width=base_param['width'],
    ...
    def _show_popup(self, b_index: int):
        ...
        self._popup_window.config(bg=self.BG_COLOR)
        ...
        for param in self.extra_params[strat_selected]:
            ...
            temp_label = tk.Label(self._popup_window, bg=self.BG_COLOR, fg=self.FG_COLOR, text=param['name'], font=self.BOLD_FONT)
            temp_label.grid(row=row_nb, column=0)

            if param['widget'] == tk.Entry:
                self._extra_input[code_name] = tk.Entry(self._popup_window, bg=self.BG_COLOR_2, justify=tk.CENTER, fg=self.FG_COLOR,
                                                        insertbackground=self.FG_COLOR, highlightthickness=False)
        ...
        validation_button = tk.Button(self._popup_window, text="Validate", bg=self.BG_COLOR_2, fg=self.FG_COLOR,
