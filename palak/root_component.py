...
logger = logging.getLogger()  # This will be the same logger object as the one configured in mainly.py


class Root(tk.Tk):
    def __init__(self, binance: BinanceClient, bitmex: BitmexClient,mod: str):
        super().__init__()
        self.mod=mod
        if mod=="on":
        ...
        self.photo = tk.PhotoImage(file="bitcoin.png")
        ...
        self._left_frame = tk.Frame(self, bg=self.BG_COLOR)
        self._left_frame.pack(side=tk.LEFT)

        self._right_frame = tk.Frame(self, bg=self.BG_COLOR)
        self._right_frame.pack(side=tk.LEFT)

        # Creates and places components at the top and bottom of the left and right frame

        self._watchlist_frame = Watchlist(self.binance.contracts, self.bitmex.contracts,self.mod, self._left_frame, bg=self.BG_COLOR)
        self._watchlist_frame.pack(side=tk.TOP, padx=10)

        self.logging_frame = Logging(self.mod,self._left_frame, bg=self.BG_COLOR)
        self.logging_frame.pack(side=tk.TOP, pady=15)  # Space a bit the components with vertical padding

        self._strategy_frame = StrategyEditor(self, self.binance, self.bitmex,self.mod, self._right_frame, bg=self.BG_COLOR)
        self._strategy_frame.pack(side=tk.TOP, pady=15)

        self._trades_frame = TradesWatch(self.mod,self._right_frame, bg=self.BG_COLOR,)
    ...
    def _save_workspace(self):
        ...
        for b_index in strat_widgets['contract']:  # Loops through the rows of a column
