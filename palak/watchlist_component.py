...
class Watchlist(tk.Frame):
    def __init__(self, binance_contracts: typing.Dict[str, Contract], bitmex_contracts: typing.Dict[str, Contract],mod
                 ,*args, **kwargs):
        ...
        self._binance_label = tk.Label(self._commands_frame, text="Binance", bg=self.BG_COLOR, fg=self.FG_COLOR, font=self.BOLD_FONT)
        self._binance_label.grid(row=0, column=0)

        self._binance_entry = Autocomplete(self.binance_symbols, self._commands_frame, fg=self.FG_COLOR, justify=tk.CENTER,
                                       insertbackground=self.FG_COLOR, bg=self.BG_COLOR_2, highlightthickness=False)
        self._binance_entry.bind("<Return>", self._add_binance_symbol)
        self._binance_entry.grid(row=1, column=0, padx=5)

        self._bitmex_label = tk.Label(self._commands_frame, text="Bitmex", bg=self.BG_COLOR, fg=self.FG_COLOR, font=self.BOLD_FONT)
        self._bitmex_label.grid(row=0, column=1)

        self._bitmex_entry = Autocomplete(self.bitmex_symbols, self._commands_frame, fg=self.FG_COLOR, justify=tk.CENTER,
                                      insertbackground=self.FG_COLOR, bg=self.BG_COLOR_2, highlightthickness=False)
        ...
        for idx, h in enumerate(self._headers):
            header = tk.Label(self._headers_frame, text=h.capitalize() if (h != "remove" or h!="Graph" or h!="Interval") else "", bg=self.BG_COLOR,
                              fg=self.FG_COLOR, font=self.GLOBAL_FONT, width=self._col_width)
            header.grid(row=0, column=idx)

        header = tk.Label(self._headers_frame, text="", bg=self.BG_COLOR,
                          fg=self.FG_COLOR, font=self.GLOBAL_FONT, width=2)
    ...
    def _add_symbol(self, symbol: str, exchange: str):
        ...
        self.body_widgets['symbol'][b_index] = tk.Label(self._body_frame.sub_frame, text=symbol, bg=self.BG_COLOR,
                                                        fg=self.FG_COLOR_2, font=self.GLOBAL_FONT, width=self._col_width)
        self.body_widgets['symbol'][b_index].grid(row=b_index, column=0)

        self.body_widgets['exchange'][b_index] = tk.Label(self._body_frame.sub_frame, text=exchange, bg=self.BG_COLOR,
                                                          fg=self.FG_COLOR_2, font=self.GLOBAL_FONT, width=self._col_width)
        ...
        self.body_widgets['bid'][b_index] = tk.Label(self._body_frame.sub_frame,
                                                     ...
                                                     bg=self.BG_COLOR, fg=self.FG_COLOR_2, font=self.GLOBAL_FONT, width=self._col_width)
        ...
        self.body_widgets['Graph'][b_index] = tk.Button(self._body_frame.sub_frame, text="Y",
                                                       bg="darkblue", fg=self.FG_COLOR, font=self.GLOBAL_FONT,
        ...
        self.body_widgets['remove'][b_index] = tk.Button(self._body_frame.sub_frame, text=" X",
                                                         bg="darkred", fg=self.FG_COLOR, font=self.GLOBAL_FONT,
